...
/** Builds the FE estimate PDF and returns the file (caller decides preview/share/image). */
fun buildFePdf(context: Context, s: FeState): java.io.File? {
    ...
    val sel = table.find { it.improvementPct == s.selectedPct } ?: return null
    ...
    return pdf.finishToFile("FE_MaX_Estimate.pdf")
}

/** Revenue MaX — themed on "Carry More. Earn More" (Higher Payload Trucks creative). */
fun buildHptPdf(context: Context, s: HptState): java.io.File? {
    val point = HptLogic.TONNAGE_POINTS[s.tonnageIdx.coerceIn(0, HptLogic.TONNAGE_POINTS.size - 1)]
    val res = HptLogic.calculate(
        HptLogic.Inputs(s.addlPayload.toD(), s.loadedPct.toD(), s.monthlyKm.toD(), s.rate.toD(), s.years.toD(5.0))
    )
    ...
    pdf.table(
        ...
        rows = listOf(
            ...
            listOf("Monthly running", num(s.monthlyKm.toD()) + " km"),
            listOf("Yearly running", num(res.yearlyKm) + " km"),
    ...
    pdf.heroBand(
        ...
        "Lifetime advantage over $yrs years: ${inrWords(res.lifetimeAdvantage)}"
    )
    pdf.note("Every extra tonne carried is direct revenue - same trip, same route, more earnings.")
    return pdf.finishToFile("Revenue_MaX_Estimate.pdf")
}

/** BRT — themed on "Ab Profit Hoga Aur Bhi Zyaada" (profit creative). */
fun buildBrtPdf(context: Context, s: BrtState): java.io.File? {
    ...
    return pdf.finishToFile("BRT_Comparison.pdf")
}
