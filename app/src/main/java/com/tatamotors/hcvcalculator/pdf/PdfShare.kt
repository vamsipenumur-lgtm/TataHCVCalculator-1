...
object PdfShare {
    ...
    /** Render every page of a PDF and stack them vertically into one tall bitmap. */
    private fun renderAllPagesStacked(pdfFile: File, scale: Int = 2): android.graphics.Bitmap? {
        val pfd = android.os.ParcelFileDescriptor.open(pdfFile, android.os.ParcelFileDescriptor.MODE_READ_ONLY)
        val renderer = android.graphics.pdf.PdfRenderer(pfd)
        val count = renderer.pageCount
        if (count == 0) { renderer.close(); pfd.close(); return null }
        // measure
        val pages = ArrayList<android.graphics.Bitmap>(count)
        var maxW = 0
        var totalH = 0
        val gap = 12 * scale
        for (i in 0 until count) {
            ...
            pages.add(bmp)
            if (w > maxW) maxW = w
            totalH += h + if (i > 0) gap else 0
        }
        renderer.close(); pfd.close()
        if (pages.size == 1) return pages[0]
        val out = android.graphics.Bitmap.createBitmap(maxW, totalH, android.graphics.Bitmap.Config.ARGB_8888)
        val c = Canvas(out)
        c.drawColor(Color.WHITE)
        var y = 0
        for ((i, p) in pages.withIndex()) {
            c.drawBitmap(p, ((maxW - p.width) / 2).toFloat(), y.toFloat(), null)
            y += p.height + gap
            if (i != 0) Unit
        }
    ...
    /** Item 23: render ALL pages of the PDF into one stacked JPG and share it (nothing cut off). */
    fun shareAsImage(context: Context, pdfFile: File, imageName: String, shareTitle: String) {
        try {
            val bmp = renderAllPagesStacked(pdfFile) ?: throw IllegalStateException("no pages")
            val dir = File(context.cacheDir, "shared").apply { mkdirs() }
    ...
    /** Item 24: render the full PDF (all pages, stacked) to a Bitmap for in-app preview. */
    fun renderFirstPage(pdfFile: File): android.graphics.Bitmap? = try {
        renderAllPagesStacked(pdfFile)
