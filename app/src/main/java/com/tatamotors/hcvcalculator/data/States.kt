...
class FeState {
    ...
    fun reset() {
        customer.reset()
        vehicle = "55T / 6.7L MAV"; monthlyKm = "10000"; mileage = "3.1"
        diesel = "100"; years = "5"; selectedPct = 7
    }
}
...
class HptState {
    ...
    var monthlyKm by mutableStateOf("5417")
    ...
    fun toJson(): JSONObject = JSONObject()
        ...
        .put("loadedPct", loadedPct).put("monthlyKm", monthlyKm)
    ...
    fun fromJson(s: String) {
        try {
            ...
            // back-compat: older saves stored yearlyKm; convert to monthly
            when {
                o.has("monthlyKm") -> monthlyKm = o.optString("monthlyKm", monthlyKm)
                o.has("yearlyKm") -> {
                    val yk = o.optString("yearlyKm").toDoubleOrNull()
                    if (yk != null) monthlyKm = (yk / 12.0).toInt().toString()
                }
            }
    ...
    fun reset() {
        customer.reset()
        tonnageIdx = 0; addlPayload = "1.8"; loadedPct = "100"
        monthlyKm = "5417"; rate = "2.5"; years = "5"
    }
}
...
class BrtState {
    ...
    fun reset() {
        customer.reset()
        product1.fromJson(ProductState("Tata Signa 2823.T").toJson())
        product2.fromJson(ProductState("Competition Model").toJson())
        compareYears = "5"
    }
}
